...
cd "/Users/ee/GitHub/rp3-aim3-medication-attitudes/sensitivity-analysis/scripts"
...
* 0. Variable Lists -------------------------------------------- *
global table1vars age sex race educ marital medicaid chronic regularmeds health dementia diagnosis hospitalized doctor adls medicationsiadl fall 

global table1outcomes pillsmax binaryattitude3 binaryattitude4

global factorvars i.age i.sex i.chronic i.regularmeds i.dementia
...
* mkdir "../data/step-3"
save "../data/step-3/imputed-data-old.dta", replace
...
* 0. Start with Imputed Data ----------------------------------- *
use "../data/step-3/imputed-data-old.dta"

global table1vars age sex race educ marital medicaid chronic regularmeds health dementia diagnosis hospitalized doctor adls medicationsiadl fall 

global table1outcomes pillsmax binaryattitude3 binaryattitude4

global factorvars i.age i.sex i.chronic i.regularmeds


* 1. Odds Ratios ----------------------------------------------- *

mkdir "../tables/log-odds-old"
...
foreach w of varlist $table1outcomes {
	...
	foreach v of varlist $table1vars {
		...
		collect export "../tables/log-odds-old/`w'.xlsx", sheet(`v'_1) modify
	}
...
foreach w of varlist $table1outcomes {
	...
	foreach v of varlist $table1vars {
		...
		collect export "../tables/log-odds-old/`w'.xlsx", sheet(`v'_2) modify
...
collect export "../tables/table2-mice-old/mice-probabilities.xlsx", name(collection_binaryattitude3_adj) sheet(binaryattitude3_adj) modify
collect export "../tables/table2-mice-old/mice-probabilities.xlsx", name(collection_binaryattitude4_adj) sheet(binaryattitude4_adj) modify
collect export "../tables/table2-mice-old/mice-probabilities.xlsx", name(collection_pillsmax_adj) sheet(pillsmax_adj) modify
...
collect export "../tables/table2-mice-old/mice-prob-se.xlsx", name(collection_binaryattitude3_se) sheet(binaryattitude3_se) modify
collect export "../tables/table2-mice-old/mice-prob-se.xlsx", name(collection_binaryattitude4_se) sheet(binaryattitude4_se) modify
collect export "../tables/table2-mice-old/mice-prob-se.xlsx", name(collection_pillsmax_se) sheet(pillsmax_se) modify
...
foreach w of varlist $table1outcomes {
	mkdir "../tables/table2-mice-old/`w'"
}
	...
	* For each of the covariates, run binary regression.
	foreach v of varlist $table1vars {
		...
		* 3 - Save p-value.
		...
		file open myfile using "../tables/table2-mice-old/`w'/`v'-mice-adj-pvalue.txt", write replace
...
collect export "../tables/table2-mice-old/bivariate-prob-se.xlsx", name(collection_binaryattitude3_se) sheet(binaryattitude3_se) modify
collect export "../tables/table2-mice-old/bivariate-prob-se.xlsx", name(collection_binaryattitude4_se) sheet(binaryattitude4_se) modify
collect export "../tables/table2-mice-old/bivariate-prob-se.xlsx", name(collection_pillsmax_se) sheet(pillsmax_se) modify
...
foreach w of varlist $table1outcomes {
	mkdir "../tables/table2-mice-old/`w'-bivariate"
}
	...
	* For each of the covariates, run binary regression.
	foreach v of varlist $table1vars {
		...
		* 3 - Save p-value.
		...
		file open myfile using "../tables/table2-mice-old/`w'-bivariate/`v'-mice-adj-pvalue.txt", write replace
		file write myfile `"`this_pvalue'"'
		file close myfile
